...
class Build(Move, ABC):
    ...
    def _build(game: "Game", card: Card) -> "Game":
        ...
        gold_paid_for_red = max(
            0,
            card.cost.red
            - current_player.production.red
            - current_player.resources.red,
        )
        gold_paid_for_green = max(
            0,
            card.cost.green
            - current_player.production.green
            - current_player.resources.green,
        )
        gold_paid_for_blue = max(
            0,
            card.cost.blue
            - current_player.production.blue
            - current_player.resources.blue,
        )
        gold_paid_for_black = max(
            0,
            card.cost.black
            - current_player.production.black
            - current_player.resources.black,
        )
        gold_paid_for_white = max(
            0,
            card.cost.white
            - current_player.production.white
        ...
        cost = AllResources(
            ...
            max(
                0,
                card.cost.green - current_player.production.green - gold_paid_for_green,
            ),
            max(
                0, card.cost.blue - current_player.production.blue - gold_paid_for_blue
            ),
            max(
                0,
                card.cost.black - current_player.production.black - gold_paid_for_black,
            ),
            max(
                0,
                card.cost.white - current_player.production.white - gold_paid_for_white,
            ),
            sum(
                (
                    gold_paid_for_red,
